...
class AnnotationSession:
    ...
    def session_path(self):
        return Path(self.dataset_root)/(Path(self.video_path).stem+".annotation.json")
    def mark_positive(self,frame_number,x,y):
        self.labels[str(int(frame_number))]={"type":"positive","x":float(x),"y":float(y),"box_size_px":int(self.box_size_px)}
    ...
    def load_or_create(cls,video_path,dataset_root):
        ...
        d=json.loads(p.read_text(encoding="utf-8"));s.current_frame=int(d.get("current_frame",0));s.box_size_px=int(d.get("box_size_px",24));s.labels=dict(d.get("labels",{}));return s
